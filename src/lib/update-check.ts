...
const CLI_NAME = "shh-env";
const NPM_PACKAGE = "shh-env";
const BREW_FORMULA = "circlesac/tap/shh-env";
// ───────────────────────────────────────────────────────────────────────────

const CACHE_DIR = join(homedir(), ".config", CLI_NAME, "cache");
const CACHE_FILE = join(CACHE_DIR, "update-check.json");
const INSTALL_MARKER = `.${CLI_NAME}-install-method`;
const SKIP_ENV = `${CLI_NAME.toUpperCase().replace(/-/g, "_")}_NO_UPDATE_CHECK`;
const REFRESH_ENV = `${CLI_NAME.toUpperCase().replace(/-/g, "_")}_INTERNAL_REFRESH`;
